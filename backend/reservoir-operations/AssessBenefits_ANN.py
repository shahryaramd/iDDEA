...
for nn in range(0,3,1):
    ...
    assessdays = 7 #number of days for asssessment
    updatestep=2
    turbc = 5340
    TW_elev = 1200  #assumed constant
    homedir = '/home/shahryar/DSS/'
    fpath = os.path.join(homedir,'ANN_Detroit/OptimData')
    ...
    url = "http://www.nwd-wc.usace.army.mil/dd/common/web_service/webexec/ecsv?id=DET.Stor.Inst.0.0.Best%3Aunits%3Dac-ft&headers=true&filename=&timezone=MDT&startdate="+sdt_lst+"+08%3A00&enddate="+sdt+"+08%3A00"
...
#		Robs.insert(2+nn,Robs[1+nn])
    ...
    he_real_ANN = list(map(lambda x:2400 if x>2400 else x, he)) #limit to 2400MWh
    ...
    ############ VIC based (copied from AssessBenefit.py) ########################################################
...
#		Robs.insert(2+nn,Robs[1+nn])
    ...
    print "Total Realized HP: ",sum(hp_real)
    he = list(map(lambda x:x*19.722,hp_real ))  #energy in MWh
    he_real_VIC = list(map(lambda x:2400 if x>2400 else x, he)) #limit to 2400MWh
    ...
    print "Total Actual HP: ",sum(hp_act)
    he = list(map(lambda x:x*19.722,hp_act ))  #energy in MWh
    he_act = list(map(lambda x:2400 if x>2400 else x, he)) #limit to 2400
    
    wrfile = os.path.join(fpath,"assess_"+curstr+".txt")     #upload to VIC path
    inflowfile = os.path.join(fpath,"obs_inflow_"+curstr+".txt")
    with open(wrfile,'w') as wrfile:
        wrfile.write("Date, VIC-based, ANN-based, Observed\n")
        for i in range(nvar):
            data = str_dt[i] + ", " + "{0:.2f}".format(float(he_real_VIC[i])) + ", " + "{0:.2f}".format(float(he_real_ANN[i])) + ", " + "{0:.2f}".format(float(he_act[i])) 
            wrfile.write("{}\n".format(data))
